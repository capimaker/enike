...
export default async function ProductsPage({
  ...
  (sp.price ? (Array.isArray(sp.price) ? sp.price : [sp.price]) : []).forEach((p) => {
    ...
    const minLabel = Number.isFinite(minVal) ? formatCurrency(minVal) : formatCurrency(0);
    const maxLabel = Number.isFinite(maxVal) ? formatCurrency(maxVal) : undefined;
    const label =
      minLabel && maxLabel ? `${minLabel} - ${maxLabel}` : minLabel && !maxLabel ? `Más de ${minLabel}` : maxLabel;
